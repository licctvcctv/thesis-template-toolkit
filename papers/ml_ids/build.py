#!/usr/bin/env python3
"""
基于机器学习的网络入侵检测系统的设计与实现论文组装器。

用法:
    cd /Users/a136/vs/45425/thesis_project
    python papers/ml_ids/build.py [输出.docx]
"""
...
IMG_DIR = HERE / "images"
TPL = ROOT / "templates" / "njgyzyjsdx" / "template.docx"
...
def insert_table_after(paragraph, table_data):
    ...
    format_table(table, table_data.get("font_size", 10.5))
...
def insert_pending_tables(doc: Document):
    for para in list(doc.paragraphs):
        ...
        prev_node = para._p.getprevious()
        prev_para = None
        while prev_node is not None:
            if prev_node.tag == qn("w:p"):
                candidate = Paragraph(prev_node, para._parent)
                if re.match(r"^表\d+-\d+\s+", candidate.text or ""):
                    prev_para = candidate
                break
            prev_node = prev_node.getprevious()
        if prev_para is not None:
            prev_para.paragraph_format.keep_with_next = True
            insert_table_after(prev_para, _pending_tables[idx])
...
def insert_formula_after(paragraph, formula_data):
    table = paragraph._parent.add_table(rows=1, cols=3, width=Mm(150))
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.autofit = False
    set_table_widths(table, [5, 122, 23])
    clear_table_borders(table)
    for cell in table.rows[0].cells:
        cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
        set_cell_margins(cell, top=10, start=20, bottom=10, end=20)

    left_para = table.rows[0].cells[0].paragraphs[0]
    left_para.paragraph_format.space_before = Pt(0)
    left_para.paragraph_format.space_after = Pt(0)

    math_para = table.rows[0].cells[1].paragraphs[0]
    math_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
    math_para.paragraph_format.space_before = Pt(0)
    math_para.paragraph_format.space_after = Pt(0)
    math_para._p.append(latex_to_omml(formula_data["latex"]))
    ...
    run = number_para.add_run(formula_data.get("number", ""))
...
def insert_pending_formulas(doc: Document):
    for para in list(doc.paragraphs):
        ...
        if not match:
            continue
        idx = int(match.group(1))
        insert_formula_after(para, _pending_formulas[idx])
        remove_paragraph(para)
...
def main() -> int:
    from build_nj_exact import main as exact_main
