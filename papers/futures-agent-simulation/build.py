...
DEFAULT_OUTPUT = HERE / "基于多智能体的期货交易模拟系统设计与实现_论文初稿.docx"
...
def add_image(doc, image_path, caption="", width_cm=12.0, page_break_before=False, page_break_after=False):
    ...
    if caption:
        ...
        fmt_run(run_cap, size=10.5, bold=True)
    if page_break_after:
        doc.add_page_break()
...
def validate_chapter_plan(chapters):
    problems = []
    if len(chapters) != 7:
        problems.append(f"论文正文应为7章，当前为{len(chapters)}章")
