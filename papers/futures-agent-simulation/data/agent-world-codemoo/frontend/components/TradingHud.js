const HUD_VERSION = 3;
const TRADING_SNAPSHOT_KEY = 'futures-agent-simulation.trading-summary.v1';
...
function injectStyles(doc) {
  ...
  style.textContent = `
    ...
    #strategy-scoreboard .sb-pool {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 7px;
    }
    #strategy-scoreboard .sb-window,
    #strategy-scoreboard .sb-pool-card {
      border-radius: 7px;
    ...
    #strategy-scoreboard .sb-pool-card b {
      display: block;
      color: #f8fafc;
      font-size: 12px;
      margin-bottom: 2px;
    ...
    #strategy-scoreboard .sb-pool-card span {
      color: #94a3b8;
      font-size: 10px;
    }
    ...
    #strategy-scoreboard .sb-agent[data-rec="adopt"] {
      border-color: rgba(74, 222, 128, 0.42);
      box-shadow: inset 3px 0 0 rgba(74, 222, 128, 0.82);
    }
    #strategy-scoreboard .sb-agent[data-rec="watch"] {
      border-color: rgba(251, 191, 36, 0.36);
      box-shadow: inset 3px 0 0 rgba(251, 191, 36, 0.80);
    }
    #strategy-scoreboard .sb-agent[data-rec="remove"] {
    ...
    #strategy-scoreboard .sb-badge.adopt { color: #052e16; background: #86efac; }
    #strategy-scoreboard .sb-badge.watch { color: #422006; background: #fde68a; }
    #strategy-scoreboard .sb-badge.remove { color: #450a0a; background: #fca5a5; }
    ...
    #prediction-report .pr-agent-card[data-rec="adopt"] { border-color: rgba(74, 222, 128, 0.42); }
    #prediction-report .pr-agent-card[data-rec="watch"] { border-color: rgba(251, 191, 36, 0.34); }
    #prediction-report .pr-agent-card[data-rec="remove"] { border-color: rgba(248, 113, 113, 0.38); }
...
export default class TradingHud {
  constructor({ i18n, document: doc = document, window: win = window } = {}) {
    ...
    console.log(`[TradingHud v${HUD_VERSION}] init`);
  ...
  _bind() {
    ...
    this._onReportClick = ev => {
      if (!ev.target?.closest?.('[data-report-toggle]')) return;
      this.reportExpanded = !this.reportExpanded;
      this._render();
    };
    this.reportRoot.addEventListener('click', this._onReportClick);
  }
  ...
  _render() {
    ...
    this.reportRoot.dataset.collapsed = this.reportExpanded ? '0' : '1';
    if (this.reportExpanded) {
      this.document.body.dataset.tradingReportOpen = '1';
    } else {
      delete this.document.body.dataset.tradingReportOpen;
    }
      ...
      <div class="th-section">
        ...
        <div class="th-prices">
          ${commodities.map(item => `
            <div class="th-price">
              <div class="th-symbol">${escapeHtml(item.name || item.symbol || item.id)}</div>
              <div class="th-value">${escapeHtml(fmtPrice(Number(prices[item.id]), locale))}</div>
            </div>
          `).join('')}
        </div>
      </div>
    ...
    this.scoreRoot.innerHTML = `
      ...
      <div class="sb-section">
        <div class="sb-label">${escapeHtml(this._t('tradingHud.strategyPool'))}</div>
        <div class="sb-pool">
          <div class="sb-pool-card"><b>${escapeHtml(pool.adopt ?? 0)}</b><span>${escapeHtml(this._t('tradingHud.poolAdopt'))}</span></div>
          <div class="sb-pool-card"><b>${escapeHtml(pool.remove ?? 0)}</b><span>${escapeHtml(this._t('tradingHud.poolRemove'))}</span></div>
        </div>
      </div>
      <div class="sb-section">
  ...
  _renderScoreCard(item, index, locale) {
    ...
    return `
      <div class="sb-agent" data-rec="${escapeHtml(status)}">
        <div class="sb-agent-head">
          <div class="sb-agent-name"><span class="sb-rank">#${index + 1}</span>${escapeHtml(item.role || item.agentId)}</div>
          <span class="sb-badge ${escapeHtml(status)}">${escapeHtml(rec.label || this._t('tradingHud.poolWatch'))}</span>
        </div>
        ...
        <div class="sb-note">${escapeHtml(rec.text || '')}</div>
  ...
  _renderReport({ trading, evaluation, locale, isCached }) {
    ...
    const agentBreakdowns = Array.isArray(analysis.agentBreakdowns) && analysis.agentBreakdowns.length
      ...
      : scoreBars.map(item => ({
        ...
        recommendationLabel: this._t('tradingHud.poolWatch')
      }));
    ...
    return `
      <div class="pr-head">
        <div class="pr-title">
          ...
          <button class="pr-toggle" type="button" data-report-toggle="1">${escapeHtml(this.reportExpanded ? this._t('tradingHud.reportCollapse') : this._t('tradingHud.reportExpand'))}</button>
        </div>
        <div class="pr-subtitle">${escapeHtml(this._t('tradingHud.reportSubtitle', {
  ...
  _renderAgentBreakdown(item, index, locale) {
    ...
    const actionTotal = Number(actions.BUY || 0) + Number(actions.SELL || 0) + Number(actions.HOLD || 0);
    const status = item.recommendation || 'watch';
    ...
    return `
      <div class="pr-agent-card" data-rec="${escapeHtml(status)}">
        <div class="pr-agent-head">
          <div>
            <div class="pr-agent-role">
              ...
              <span class="pr-agent-badge">${escapeHtml(item.recommendationLabel || this._t('tradingHud.poolWatch'))}</span>
            </div>
  ...
  destroy() {
    ...
    this.reportRoot?.removeEventListener('click', this._onReportClick);
    delete this.document.body.dataset.tradingReportOpen;
