...
TPL = os.path.join(ROOT, "templates/shjkyxy/template.docx")
...
def _format_reference(ref, index):
    """Return reference text with [N] numbering."""
    text = str(ref).strip()
    if not text:
        return text
...
def main():
    ...
    if not output:
        ...
        output = os.path.join(HERE, f"中药方剂推荐_{ts}.docx")
...
def _fix_keyword_runs(doc):
    """Fix keyword paragraph formatting for shjkyxy template.

    Original template format:
    - Chinese: run0="关键词"(bold) + run1="：" + run2="内容"(not bold), firstLine=480
    - English: run0="KEY WORDS"(bold) + run1=": " + run2="内容"(not bold), firstLine=0
    """
    from docx.oxml import OxmlElement
    ...
    for p in doc.paragraphs:
        ...
        if text.startswith("关键词"):
            # Split: "关键词：xxx" → bold("关键词") + normal("：xxx")
            content = text[len("关键词"):]
            # Clear all existing runs
            for r in p.runs:
                r.text = ""
            # Save first run's rPr as base
            base_rPr = None
            if p.runs:
            ...
            if p.runs:
                ...
                r0.text = "关键词"
                r0.font.bold = True
            # Add run1 = "：内容" (not bold)
            r1_el = OxmlElement('w:r')
            if base_rPr is not None:
                new_rPr = copy.deepcopy(base_rPr)
                # Remove bold
                b = new_rPr.find(qn('w:b'))
                if b is not None:
                    new_rPr.remove(b)
                r1_el.append(new_rPr)
            t1 = OxmlElement('w:t')
        ...
        elif text.upper().startswith("KEY WORDS"):
            content = text[len("KEY WORDS"):]
            for r in p.runs:
                r.text = ""
            base_rPr = None
            if p.runs:
            ...
            r1_el.append(t1)
            p._p.append(r1_el)
            # Remove firstLine indent for English keywords
            pPr = p._p.pPr
            if pPr is not None:
