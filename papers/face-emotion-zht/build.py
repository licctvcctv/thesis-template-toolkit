...
def main():
    ...
    # 清理 Word 里损坏的交叉引用域，避免显示“Reference source not found”
    _strip_broken_ref_fields(output)
    # 最终稿不保留模板中的审阅批注，避免 Word 右侧出现批注栏
...
def _post_process(docx_path):
    ...
    page_break_pat = re.compile(r'^__PAGE_BREAK__$')
...
def _strip_review_comments(docx_path):
    ...
    def strip_markup(root):
        removed = 0
        for tag in comment_tags:
            for el in reversed(root.xpath(f".//w:{tag}", namespaces=ns)):
                parent = el.getparent()
    ...
    print("  批注清理: "
          f"{stats['anchors']}个锚点, {stats['parts']}个部件")


def _strip_broken_ref_fields(docx_path):
    """把损坏的 REF 交叉引用域转成静态文本，避免 Word 显示引用错误。"""
    import os
    import re
    import tempfile
    import zipfile
    ...
    def is_ref_instr(run):
        instr = run.find(tag("instrText"))
        if instr is None:
            return False
        text = "".join(t.text or "" for t in run.findall(tag("instrText")))
        return bool(re.search(r"\bREF _Ref\d+", text))

    def strip_container(parent):
        removed = 0
        ...
        while i < len(children):
            ...
            if child.tag == tag("r"):
                ...
                if fld is not None and fld.get(f"{{{w_ns}}}fldCharType") == "begin":
                    ...
                    while j < len(children):
                        ...
                        if node.tag != tag("r"):
                            j += 1
                            continue
                        if is_ref_instr(node):
                            is_ref = True
                            instr_run = node
                        fld_node = node.find(tag("fldChar"))
                        if fld_node is not None and is_ref:
                            fld_type = fld_node.get(f"{{{w_ns}}}fldCharType")
                            if fld_type == "separate":
                                ...
                                break
                        j += 1
                    if is_ref and instr_run is not None and sep_run is not None and end_run is not None:
                        for run in (child, instr_run, sep_run, end_run):
                ...
                removed += strip_container(child)
            i += 1
        return removed

    stats = {"fields": 0, "parts": 0}
    ...
    try:
        with zipfile.ZipFile(docx_path, "r") as zin:
            overrides = {}
            for name in zin.namelist():
                if not story_pat.match(name):
                    continue
                root = etree.fromstring(zin.read(name))
                removed = strip_container(root)
                if removed:
                    stats["fields"] += removed // 4
                    overrides[name] = xml_bytes(root)

            if not overrides:
                print("  引用域清理: 0个REF字段, 0个部件")
                return

            stats["parts"] = len(overrides)
    ...
    print("  引用域清理: "
          f"{stats['fields']}个REF字段, {stats['parts']}个部件")
