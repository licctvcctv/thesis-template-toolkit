            ...
            if info.filename == "word/settings.xml":
                ...
                for instr in root.xpath(".//w:instrText[contains(., 'TOC')]", namespaces=ns):
                    ...
                    if paragraph is None:
                        continue
                    for fld in paragraph.xpath(".//w:fldChar[@w:fldCharType='begin']", namespaces=ns):
                        fld.set(qn("w:dirty"), "true")
                        break
                replacements[info.filename] = etree.tostring(
...
def build_data(doc):
    ...
    # WPS/Word and LibreOffice paginate this thesis differently.  Cached TOC page
    # maps generated by headless LibreOffice can show 100+ pages even when WPS
    # opens the same DOCX at about 60 pages, so the default deliverable keeps a
    # real updateable TOC field and lets the target editor calculate pages.
    toc_entries = load_json("toc_pages.json") if os.environ.get("TRAFFIC_BIGDATA_USE_CACHED_TOC") == "1" else []
